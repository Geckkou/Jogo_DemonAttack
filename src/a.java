...
public class a extends Applet implements Runnable {
	...
	public void run() {
		...
		BufferedImage[] sprites = new BufferedImage[32];
		BufferedImage[][] enemySprites = new BufferedImage[256][12];
		BufferedImage image = new BufferedImage(160, 210, 1);
		...
		for(i = 0; i < 15; i++) {
			...
			for(y = 0; y < k; y++) {
				...
				for(x = 0; x < 8; x++) {
					pixels[15 - x] = pixels[x] = ((j & 1) == 0) ? 0 : 
						i < 14 ? scoreColor : playerColor;
					j >>= 1;
				}
		...
		for(k = 0; k < 256; k++) {
			...
			for(i = 0; i < 12; i++) {
				...
				for(y = 0; y < 8; y++) {
					j = S.charAt(i < 3 ? 76 + 12 * ((k >> 1) % 6) + (i << 2) + (y >> 1)
							: 148 + ((i - 3) << 2) + (y >> 1));
					if ((y & 1) == 1) {
						j >>= 8;
					}
		...
		for(i = 0; i < 120; i++) {
			for(j = 0; j < 7; j++) {
				...
				floorColors[i][j] = new Color(Color.HSBtoRGB(
						(234 + j - (i << 1)) / 360f,
						0.51f + 0.08f * j,
						0.94f - 0.06f * j - 0.25f * w * w));
			}
		}

		// Create extra lives colors
		for(i = 0; i < 100; i++) {
			extraLivesColors[i] = new Color(Color.HSBtoRGB(i / 100f, 0.46f,
					0.65f + 0.35f * (float)Math.cos(0.594245f + 0.35f * i)));
		}
		...
		while(true) {
			...
			do {
				...
				for(i = bullets.size() - 1; i >= 0; i--) {
					...
					bullet[BULLET_X] = bullet[BULLET_DEMON_X] + bullet[BULLET_OFFSET_X]
							+ ((Math.sin(bullet[BULLET_ANGLE]) > 0.5) 
					...
					if (playerExploding == 0
							&& bullet[BULLET_Y] <= 185
							&& bullet[BULLET_Y] >= 175
							&& bullet[BULLET_X] >= playerX
							&& bullet[BULLET_X] <= playerX + 7) {
						bullets.remove(i);
					    if (!cheat)
						playerExploding = 63;
					}
				...
				if (noShootDelay > 0) {
					...
					if (shooting) {
						...
						shooting = true;
						shootCounter = (lasers ? 6 : 3)
								+ (level < 10 ? (level >> 1) : 5)
								+ random.nextInt(lasers ? 6 : 3);
						shootTimer = 1;
					}
				}
				...
				if (nextLevelDelay > 0) {
					if (gameOver == 0 && --nextLevelDelay == 0) {
						...
						demonSpeed = (level <= 11) ? 0.4f + 0.2f * level / 11
								: 0.6f + 0.2f * (level - 11) / 72;
				...
				} else if (extraDemons == 0 && demons.size() == 0
						&& playerExploding == 0 && bullets.size() == 0) {
					nextLevelDelay = 99;
				} else  {
					...
					if (level > 3) {
						...
						if (demonsInRows[2].size() == 1) {
							...
							if (demon[DEMON_BIRD] == 1 && demon[DEMON_DIVING] == 0
									&& demonsInRows[3].size() == 0) {
								demonsInRows[2].remove(0);
								demonsInRows[3].add(demon);
								demon[DEMON_DIVING_Y] = demon[DEMON_Y];
								demon[DEMON_DIVING] = 1;
								demon[DEMON_PERCENT_X] = 0;
								demon[DEMON_DELTA_X] = demon[DEMON_X] < playerX
						...
						if (demon[DEMON_HIDDEN] > 0) {
							...
							if (demon[DEMON_DIVING] == 1) {
								...
								demon[DEMON_Y] = demon[DEMON_DIVING_Y] 
										+ 16 * (float)Math.sin(3.14f * demon[DEMON_PERCENT_X]);
								...
								if (playerExploding == 0
										&& demon[DEMON_Y] >= 171
										&& demon[DEMON_Y] <= 185
										&& demon[DEMON_X] + 7 >= playerX
										&& demon[DEMON_X] <= playerX + 7) {
									demons.remove(i);
								...
								boolean shooter = demon[DEMON_ROW] == 2
										&& (demon[DEMON_BIRD] == 0 || demon[DEMON_SHOOTER] == 1)
										&& demonsInRows[3].size() == 0;
								...
								if (shooting && shooter) {
									if((shoot)&&(!cheat)) {
										...
										if (lasers || random.nextBoolean()) {
											...
											bullet[BULLET_X] = demon[DEMON_X]
													+ (bullet[BULLET_OFFSET_X]
															= (lasers || demon[DEMON_BIRD] == 0 ? 4 : 0)
															+ (lasers ? 0 : random.nextInt(3)));
											bullet[BULLET_Y] = demon[DEMON_Y] + 8;
											bullet[BULLET_RADIUS] = demon[DEMON_BIRD] == 1 ? 1
													: lasers ? 0 : 1;
											...
											if (demon[DEMON_BIRD] == 0 || !lasers) {
												...
												bullet2[BULLET_X] = demon[DEMON_X] +
														(bullet2[BULLET_OFFSET_X]
																= (demon[DEMON_BIRD] == 1 ? 0 : 4)
																+ (lasers ? 3 : random.nextInt(3))
																+ 4);
									...
									if (demon[DEMON_PERCENT_Y] >= 1) {
										...
										demon[DEMON_DELTA_PERCENT_Y]
												= 1.57f / (30 + random.nextInt(15));
										demon[DEMON_START_Y] = demon[DEMON_Y];
										demon[DEMON_DELTA_Y] = 40 + ((int)demon[DEMON_ROW] << 5)
												+ random.nextInt(demon[DEMON_ROW] == 2 ? 14 : 30)
												- demon[DEMON_Y];
										float dy = demon[DEMON_DELTA_Y] < 0 ? -demon[DEMON_DELTA_Y]
												: demon[DEMON_DELTA_Y];
										if (dy < 1) {
											dy = 1;
										}
										demon[DEMON_DELTA_PERCENT_Y]
												= (0.9f + 0.1f * random.nextFloat())
												* demonSpeed * 1.57f / dy;
									} else {
										demon[DEMON_PERCENT_Y] += demon[DEMON_DELTA_PERCENT_Y];
										demon[DEMON_Y] = demon[DEMON_START_Y]
												+ demon[DEMON_DELTA_Y] * (float)Math.sin(
														demon[DEMON_PERCENT_Y]);
									}
									...
									if (demon[DEMON_PERCENT_X] >= 1) {
										...
										float dx = demon[DEMON_DELTA_X] < 0 ? -demon[DEMON_DELTA_X]
												: demon[DEMON_DELTA_X];
										if (dx < 1) {
											dx = 1;
										}
										demon[DEMON_DELTA_PERCENT_X]
												= (0.9f + 0.1f * random.nextFloat())
												* demonSpeed * 1.57f / dx;
									} else {
										demon[DEMON_PERCENT_X] += demon[DEMON_DELTA_PERCENT_X];
										demon[DEMON_X] = demon[DEMON_START_X]
												+ demon[DEMON_DELTA_X] * (float)Math.sin(
														demon[DEMON_PERCENT_X]);
										if (shooter && level > 7) {
							...
							if (playerBulletY < 175
									&&  playerBulletY <= demon[DEMON_Y] + 7
									&& playerBulletY + 7 >= demon[DEMON_Y]
											&& playerBulletX >= demon[DEMON_X]
													+ (demon[DEMON_BIRD] == 0 && tinyEnemies ? 4 : 0)
													&& playerBulletX <= demon[DEMON_X]
															+ (demon[DEMON_BIRD] == 1 
															? 7 : (tinyEnemies ? 11 : 15))) {
			...
			// -- render starts ------------------------------------------------------
			Color c = new Color(50,50,5);


			// draw background
			Color backgroundColor = playerExploding > 47 ? explodingFlashColors[63 - playerExploding] : c; g.setColor(backgroundColor);

			g.setColor(backgroundColor);
			g.fillRect(0, 20, 160, 168);
			...
			if (gameOver == 0) {
				...
				for(i = bullets.size() - 1; i >= 0; i--) {
					float[] bullet = bullets.get(i);
					g.fillRect((int)bullet[BULLET_X], (int)bullet[BULLET_Y], 1, 4);
				}
				...
				for(i = demons.size() - 1; i >= 0; i--) {
					...
					if (demon[DEMON_EXPLODING] == 1) {
						if (demon[DEMON_BIRD] == 1) {
							g.drawImage(
									enemySprites[level][6 + (int)demon[DEMON_EXPLODING_INDEX]],
									(int)demon[DEMON_X],
									(int)demon[DEMON_Y],
									(int)demon[DEMON_X] + 8,
									(int)demon[DEMON_Y] + 8,
							...
							g.drawImage(
									enemySprites[level][(level > 3 ? 6 : 3)
									                    + (int)demon[DEMON_EXPLODING_INDEX]],
									(int)demon[DEMON_X],
									(int)demon[DEMON_Y], null);
						}
					} else if (demon[DEMON_FORMING] == 1) {
						g.drawImage(enemySprites[level][j + 3],
								(int)demon[DEMON_X], (int)demon[DEMON_Y],
								(int)demon[DEMON_X] + 32, (int)demon[DEMON_Y] + 8,
								0, 0, 8, 8, null);
						g.drawImage(enemySprites[level][j + 3],
								7, 0, 15, 8, null);
					} else {
						g.drawImage(enemySprites[level][
						                                demon[DEMON_BIRD] == 1 ? 9 + j : j],
								(int)demon[DEMON_X],
								(int)demon[DEMON_Y], null);
					}
				}
				...
				if (playerExploding > 0) {
					...
					g.drawImage(sprites[(playerExploding >> 3) + 15],
							(int)playerX - 4, 153, null);
				} else {
					// draw player bullet
					g.setColor(playerBulletColor);
					g.fillRect(playerBulletX, (int)playerBulletY, 1, 8);

					// draw player's ship
					g.drawImage(sprites[14], (int)playerX, 174, null);
				}
			}
			...
			if (demoMode && score == 0) {
				// draw IMAGIC logo
				for(i = 0; i < 4; i++) {
					g.drawImage(sprites[10 + i], 63 + (i << 3), 6, null);
				}
			} else {
				// draw score
				j = score;
				x = 95;
				do {
					g.drawImage(sprites[j % 10], x, 6, null);
					x -= 8;
					j /= 10;
				} while(j > 0);
			}
			...
			if (level < 84) {
				// draw ground
				for(i = 0; i < 7; i++) {
					g.setColor(floorColors[gameOver][i]);
					g.fillRect(0, 188 + i, 160, 1);
				}
				g.fillRect(0, 195, 160, 15);

				if (!demoMode) {
					// draw extra lives
					g.setColor(extraLivesColors[nextLevelDelay]);
					for(i = 0; i < extraLives; i++) {
						g.fillRect(17 + (i << 3), 190, 1, 3);
						g.fillRect(18 + (i << 3), 188, 1, 3);
						g.fillRect(19 + (i << 3), 190, 1, 3);
					}
				}
			} else {
				// draw kill screen
				g.setColor(backgroundColor);
				g.fillRect(0, 20, 160, 190);
			}

			g.setColor(Color.WHITE);
			g.setFont(new Font("Monotype Corsiva",Font.ITALIC, 12));
			g.drawString("Demon Attack",3 ,203 );

			g.setFont(new Font("Impact",0, 10));
			g.drawString("Recorde:"+recorde,78 ,203 );

			if (cheat)  {
		          g.setColor(Color.WHITE);
		          g.setFont(new Font("Monotype Corsiva",Font.ITALIC, 12));
		          g.drawString("trapaça",123 ,203 );
			}
			...
			if (g2 == null) {
				...
				g2.drawImage(image, 0, 0, 640, 420, null);
			}
	...
	public void processKeyEvent(KeyEvent keyEvent) {
		...
		final int VK_LIGA_TRAPACA = 84;
		final int VK_DESLIGA_TRAPACA = 85;  
		...
		if (k == VK_LIGA_TRAPACA)  {
			cheat = true;

		} else if (k == VK_DESLIGA_TRAPACA)  {
			cheat = false;
		}
		...
		if (k > 0) {
			...
			a[(k == VK_LEFT || k == VK_RIGHT || k == VK_P || k == VK_LIGA_TRAPACA || k == VK_DESLIGA_TRAPACA) ? k : VK_SHOOT]
					= keyEvent.getID() != 402;
		}
	}
